# Claude Code 抓包包装(bash / zsh)
#
# 安装:
#     echo "source /ABS/PATH/TO/myproxy/shell/claude-trace.sh" >> ~/.bashrc   # bash
#     echo "source /ABS/PATH/TO/myproxy/shell/claude-trace.sh" >> ~/.zshrc    # zsh
# 或临时:
#     source /ABS/PATH/TO/myproxy/shell/claude-trace.sh
#
# 提供两个命令:
...
: "${CLAUDE_TRACE_PORT:=8888}"
: "${CLAUDE_TRACE_CA:=$HOME/.mitmproxy/mitmproxy-ca-cert.pem}"

claude-trace() {
    if ! nc -z 127.0.0.1 "$CLAUDE_TRACE_PORT" 2>/dev/null; then
    ...
    env -u ALL_PROXY -u all_proxy \
        ...
        command claude $CLAUDE_TRACE_ARGS "$@"
}
